...
import { Send, Lock, DollarSign, CheckCircle, AlertCircle, Loader2, User, ArrowUpCircle, Search } from "lucide-react"
...
export default function FundTransferPage() {
  const {
    ...
    setError,
    setValue,
  } = useForm<FundTransferFormData>({ mode: "onChange" })
  ...
  const onSubmit = async (data: FundTransferFormData) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/account/fund-transfer`,
        {
          amount: data.amount,
          pin: data.pin,
      ...
      setTimeout(() => {   
        toast.success(response.data.msg || "Funds transferred successfully")
        reset()
    ...
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-white flex flex-col items-center justify-center p-4">
      ...
      >
        <div className="relative overflow-hidden rounded-3xl bg-white shadow-2xl">
          {/* Header */}
          <div className="bg-gradient-to-r from-indigo-600 to-blue-500 p-6 pb-16">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold text-white">Fund Transfer</h2>
              <div className="h-12 w-12 rounded-full bg-white/20 flex items-center justify-center">
                <Send className="h-6 w-6 text-white" />
              </div>
            </div>
            <p className="text-indigo-100 mt-1">Send money to another account securely</p>
          </div>
              ...
              {success ? (
                ...
                >
                  ...
                  <p className="text-gray-600 text-center">
                    Your funds have been transferred successfully. Redirecting to dashboard...
                  </p>
                </motion.div>
              ) : (
                <motion.form
          ...
          <div className="p-6">
            <div className="rounded-xl bg-indigo-50 p-4">
              <h3 className="font-medium text-indigo-800 mb-2">Transfer Information</h3>
              <ul className="space-y-2 text-sm text-gray-600">
                <li className="flex items-start">
                  ...
                  <span>Transfers are processed immediately between accounts</span>
                </li>
                <li className="flex items-start">
                  <span className="h-5 w-5 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center mr-2 mt-0.5 flex-shrink-0">
                    2
                ...
                <li className="flex items-start">
                  ...
                  <span>Double-check the recipient account number before confirming</span>
                </li>
              </ul>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[500, 1000, 2000, 5000, 10000, 20000].map((quickAmount) => (
                <button
                  ...
                  className="flex items-center justify-center gap-1 py-2 px-3 bg-indigo-100 hover:bg-indigo-200 rounded-lg text-indigo-700 text-sm font-medium transition-colors"
                >
